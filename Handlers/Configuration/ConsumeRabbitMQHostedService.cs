...
namespace Tracnghiem.Handlers.Configuration
{
    public class ConsumeRabbitMQHostedService : BackgroundService
    {
        ...
        public ConsumeRabbitMQHostedService(IPooledObjectPolicy<IModel> objectPolicy, IConfiguration Configuration, IServiceProvider ServiceProvider)
        {
        ...
        private async Task HandleMessage(string SourceModuleName, string routingKey, string content)
        {
            using var scope = ServiceProvider.CreateScope();
            foreach (IHandler handler in Handlers)
            {
                handler.ServiceProvider = scope.ServiceProvider;
                handler.SourceModuleName = SourceModuleName;
            }
