...
namespace Tracnghiem.Rpc.app_user
{
    ...
    {
        [Route(AppUserRoute.SingleListImage), HttpPost]
        public async Task<List<AppUser_ImageDTO>> SingleListImage([FromBody] AppUser_ImageFilterDTO AppUser_ImageFilterDTO)
        {
        ...
        [Route(AppUserRoute.SingleListExam), HttpPost]
        public async Task<List<AppUser_ExamDTO>> SingleListExam([FromBody] AppUser_ExamFilterDTO AppUser_ExamFilterDTO)
        {
            if (!ModelState.IsValid)
                throw new BindException(ModelState);
